...
@app.route('/')
def index():
    # Using a simple string here to avoid f-string curly brace errors
    return '''
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        ''' + POP_UNDER + '''
        <style>
            body { font-family: 'Segoe UI', sans-serif; background: #0f0f0f; color: #eee; padding: 20px; text-align: center; }
            .box { background: #1a1a1a; padding: 30px; border-radius: 12px; border: 1px solid #333; max-width: 450px; margin: auto; }
            input { width: 90%; padding: 12px; margin: 15px 0; background: #252525; color: #fff; border: 1px solid #444; border-radius: 6px; }
...
def search():
    ...
    try:
        ...
        # Build HTML safely using addition to avoid f-string crashes
        html = '''
        <head>
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            ''' + POP_UNDER + '''
        ...
        <body>
            ...
            <h2>Search Results: ''' + str(query) + '''</h2>
        '''
