...
# Vercel looks for these two lines first. Do not move them.
app = Flask(__name__)
application = app

# --- AD CONFIGURATION ---
TOP_AD = '<div style="background:#222; margin:15px 0; padding:15px; border:1px dashed #4dabf7; color:#4dabf7; text-align:center; border-radius:8px;">✨ ADVERTISEMENT: CLICK TO UNLOCK PREMIUM FEATURES ✨</div>'
        ...
        if json_resp.get("success"):
            ...
            for cat, records in data_content.items():
                if isinstance(records, list):
                    for item in records:
                        found = True
                        if count % 2 == 0 and count != 0: output += FEED_AD
                        
                        # Universal Number Search
                        ...
                        count += 1
            if not found: return f"{style}<h2>No Records.</h2><a href='/'>Back</a>"
